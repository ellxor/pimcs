...
typedef complex double WaveVector[NumberOfEmitters + 1][CavityTruncation];
	...
	for (int64 n = state->rowb; n <= state->rowa; ++n) { // number of spin down
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			...
			dest[n][a] -= source[n][a] * state->time_step/2 * (
				...
				config.CavityEmissionRate * (NumberOfEmitters - n) * (a + 1 + cnorm(state->alpha)) +
				config.CavityAbsorptionRate * n * (a + cnorm(state->alpha))
			);

			dest[n][a - 1] -= source[n][a] * state->time_step/2 * conj(state->alpha) * sqrt(a) * (
				config.CavityAbsorptionRate * n +
				config.CavityEmissionRate * (NumberOfEmitters - n)
			);

			if (a + 1 < CavityTruncation) dest[n][a + 1] -= source[n][a] * state->time_step/2 * state->alpha * sqrt(a + 1) * (
				config.CavityAbsorptionRate * n +
				config.CavityEmissionRate * (NumberOfEmitters - n)
			);

			// QSD correction term
			dest[n][a - 1] += source[n][a] * config.PhotonLossRate * state->time_step * conj(state->annihilation) * sqrt(a);
			hamiltonian_term(dest, source, state, n, a);

			if (UseDisplacement) {
				complex double shift = (state->alpha * config.PhotonLossRate * state->time_step) / 2;

				if (a - 1 >= 0)               dest[n][a - 1] += source[n][a] * conj(shift) * sqrt(a);
				if (a + 1 < CavityTruncation) dest[n][a + 1] -= source[n][a] *      shift  * sqrt(a + 1);
			}
		}
	}
}
...
void evolve_under_H_eff(WaveVector wave, struct TrajectoryState *state) {
	...
	complex double (*a)[CavityTruncation] = wave; // Controlled by pointers which are cheap to swap.
	complex double (*b)[CavityTruncation] = _b.wave;
	...
	for (int64 i = 1; i <= config.RungeKuttaPoly; ++i) {
		...
		complex double (*tmp)[CavityTruncation] = a;
		a = b;
		b = tmp;
	}

	complex double dxi = random_complex_gaussian(state->time_step);

	for (int64 n = state->rowb; n <= state->rowa; ++n) {
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			wave[n][a - 1] += wave[n][a] * sqrt(config.PhotonLossRate * a) * dxi;
			// wave[n][a] -= wave[n][a] * sqrt(config.PhotonLossRate) * state->annihilation * dxi; //(norm shrinkage)
		}
	}
}


void linear_displacement_step(WaveVector dest, WaveVector source, struct TrajectoryState *state, complex double shift) {
	memset(dest, 0, sizeof(WaveVector));
...
void displace(WaveVector wave, struct TrajectoryState *state, complex double shift) {
	// In this case of an exponential and linear Hamiltonian, the Runge-Kutta method
	// is identical to a Taylor series expansion, so this is performed directly for efficiency.
	...
	const int64 TaylorSeriesTerms = 50;
...
void jump_spin_loss_same_j(WaveVector wave, struct TrajectoryState *state) {
	for (int64 n = state->rowa; n >= state->rowb; --n) { // iterate backwards due to overlap
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			...
			wave[n + 1][a] = sqrt((jmm + 1)*jpm) * wave[n][a];
		}
	}
...
void jump_spin_loss_lower_j(WaveVector wave, struct TrajectoryState *state) {
	for (int64 n = state->rowa; n >= state->rowb; --n) { // iterate backwards due to overlap
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			...
			wave[n + 1][a] = sqrt((jpm - 1)*(jpm)) * wave[n][a];
		}
	}
...
void jump_spin_loss_upper_j(WaveVector wave, struct TrajectoryState *state) {
	for (int64 n = state->rowa; n >= state->rowb; --n) { // iterate backwards due to overlap
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			...
			wave[n + 1][a] = sqrt((jmm + 1)*(jmm + 2)) * wave[n][a];
		}
	}
...
void jump_spin_gain_same_j(WaveVector wave, struct TrajectoryState *state) {
	for (int64 n = state->rowb; n <= state->rowa; ++n) {
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			...
			wave[n - 1][a] = sqrt((jpm + 1)*jmm) * wave[n][a];
		}
	}
...
void jump_spin_gain_lower_j(WaveVector wave, struct TrajectoryState *state) {
	for (int64 n = state->rowb; n <= state->rowa; ++n) {
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			...
			wave[n - 1][a] = sqrt((jmm - 1)*(jmm)) * wave[n][a];
		}
	}
...
void jump_spin_gain_upper_j(WaveVector wave, struct TrajectoryState *state) {
	for (int64 n = state->rowb; n <= state->rowa; ++n) {
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			...
			wave[n - 1][a] = sqrt((jpm + 1)*(jpm + 2)) * wave[n][a];
		}
	}
...
void jump_photon_loss(WaveVector wave, struct TrajectoryState *state) {
	for (int64 n = state->rowb; n <= state->rowa; ++n) {
		for (int64 a = state->mina; a <= state->maxa; ++a) {
			wave[n][a - 1] += sqrt(a) * wave[n][a];
			wave[n][a] *= state->alpha;
		}
	}
...
void jump_photon_gain(WaveVector wave, struct TrajectoryState *state) {
	for (int64 n = state->rowb; n <= state->rowa; ++n) {
		for (int64 a = state->maxa; a >= state->mina; --a) {
			if (a + 1 < CavityTruncation) wave[n][a + 1] += sqrt(a + 1) * wave[n][a];
			wave[n][a] *= conj(state->alpha);
		}
	}
...
double normalize_state(WaveVector wave, struct TrajectoryState *state, int64 choice) {
	double norm = 0;
	...
	if (!norm) printf("Last choice: %lld\n", choice);
...
struct TrajectoryState simulate_trajectory(struct TrajectoryState *initial, double start_time, double end_time, bool output) {
	...
	int64 row1 = (int)(NumberOfEmitters/2.0 + initial_j_sector);
	...
	complex double (*wave)[CavityTruncation] = *state.wave;
	...
	while (state.time <= config.EndTime) {
		if (output && state.time == next_write) {
			...
			if (step == OutputCount - 1) next_write = config.EndTime; // fix rounding error on last output
 		}

		double jump_table[JUMP_COUNT] = {0};
		state.annihilation = 0;

		static thread_local struct WaveVectorAllocation tmp_loss_state;
		static thread_local struct WaveVectorAllocation tmp_gain_state;

		memcpy(tmp_loss_state.wave, wave, sizeof(WaveVector));
		memcpy(tmp_gain_state.wave, wave, sizeof(WaveVector));

		struct TrajectoryState tmp_state = state;
		jump_photon_loss(tmp_loss_state.wave, &tmp_state);
		jump_photon_gain(tmp_gain_state.wave, &tmp_state);
		...
		for (int64 n = state.rowb; n <= state.rowa; ++n) {
			...
			for (int64 a = state.mina; a <= state.maxa; ++a) {
				...
				state.annihilation += conj(wave[n][a - 1]) * wave[n][a] * sqrt(a);
			}

            for (int64 a = tmp_state.mina; a < tmp_state.maxa; ++a) {
                float photon_gain_factor = cnorm(tmp_gain_state.wave[n][a]);
                float photon_loss_factor = cnorm(tmp_loss_state.wave[n][a]);

				jump_table[JUMP_SPIN_LOSS_PHOTON_GAIN_SAME_J]  += (jmm + 1)*(jpm) * photon_gain_factor;
				jump_table[JUMP_SPIN_LOSS_PHOTON_GAIN_LOWER_J] += (jpm - 1)*(jpm) * photon_gain_factor;
				jump_table[JUMP_SPIN_LOSS_PHOTON_GAIN_UPPER_J] += (jmm + 1)*(jmm + 2) * photon_gain_factor;

 				jump_table[JUMP_SPIN_GAIN_PHOTON_LOSS_SAME_J]  += (jpm + 1)*(jmm) * photon_loss_factor;
 				jump_table[JUMP_SPIN_GAIN_PHOTON_LOSS_LOWER_J] += (jmm - 1)*(jmm) * photon_loss_factor;
 				jump_table[JUMP_SPIN_GAIN_PHOTON_LOSS_UPPER_J] += (jpm + 1)*(jpm + 2) * photon_loss_factor;
            }
		}

		for (int64 i = 0; i < JUMP_COUNT; ++i) {
			assert(jump_table[i] >= 0);
		}
		...
		if (UseDisplacement) {
			...
			for (int64 n = state.rowb; n <= state.rowa; ++n) {
				for (int64 a = state.mina; a <= state.maxa; ++a) {
					annihilation += conj(wave[n][a - 1]) * wave[n][a] * sqrt(a);

				}
			}

			if (cnorm(annihilation) > 0.02) {
				double ampl = cnorm(annihilation);
				const double threshold = 1.0;

				if (ampl >= threshold) fprintf(stderr, "displacement: %g\n", ampl);
// 				assert(ampl < threshold);

				displace(wave, &state, annihilation);
			}
		}
		...
		for (;; ++state.rowb) {
			double norm = 0;
			for (int64 a = state.mina; a <= state.maxa; ++a) norm += cnorm(wave[state.rowb][a]);
		...
		for (;; --state.rowa) {
			double norm = 0;
			for (int64 a = state.mina; a <= state.maxa; ++a) norm += cnorm(wave[state.rowa][a]);
			if (norm >= config.ShrinkTolerance) break;
		...
		if (!UseDisplacement) { // only shrink if state is not displaced!
			for (;; ++state.mina) {
				double norm = 0;
				for (int64 n = state.rowb; n <= state.rowa; ++n) norm += cnorm(wave[n][state.mina]);
				if (norm >= config.ShrinkTolerance) break;
			...
			for (;; --state.maxa) {
				double norm = 0;
				for (int64 n = state.rowb; n <= state.rowa; ++n) norm += cnorm(wave[n][state.maxa]);
				if (norm >= config.ShrinkTolerance) break;
...
void two_time_correlation() {
	...
	for (int64 split = 0; split < 4; ++split) { // cycle through 1, i, -1, -i
		...
		double norm = normalize_state(*phi.wave, &phi, 0);
