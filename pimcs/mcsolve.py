...
def mcsolve(system: Dicke, psi0: DickeState, tlist: list[float], e_ops=[], ntraj: int = 0, ncpu: int = 0,
    ...
    dt = tlist[1] - tlist[0]
    assert np.isclose(tlist[1:] - tlist[:-1],
                      dt).all() and dt > 0, "tlist must be monotonically increasing and linearly spaced"
    ...
    code, spin_width, boson_width, tfuncs = c.generate_backend_code(system.hamiltonian, e_ops, tfunc_tlist,
                                                                    displace=enable_displacement,
                                                                    two_time_correlation=correlation_time is not None)
    config = c.generate_config(system, boson_dim, tlist, len(e_ops), ntraj, ncpu, jtol, stol, spin_width, boson_width,
                               len(tlist), rkpoly, tfunc_points, enable_displacement)
    ...
    if correlation_time is not None and not use_conservation:
        ntraj *= 4
